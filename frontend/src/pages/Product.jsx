...
const Product = () => {
  ...
  useEffect(()=>{
    const getProduct=async()=>{
        try{
            const res=await puplicRequest.get('/products/find/'+id)
            setProduct(res.data)
        }catch(err){
    ...
    <InfoContainer>
        ...
        <FilterContainer>
            <Filter>
                ...
                {product.color?.map((c,ix)=>(
                    <FilterColor color={c}  key={ix} onClick={()=>setColor(c)}/>
                ))}
            ...
            <FilterSize onChange={(e)=>setSize(e.target.value)}>
                {product.size?.map((s,ix)=>(
                    <FliterSizeOption key={ix}>{s}</FliterSizeOption>
                ))}
